let bundeslaender = [
          ...
          },
          {
                    ...
                    comments: [
                              "Die Stadtmusikanten haben mir schon immer gefallen!",
                    ],
          },
          {
                    name: "Hamburg",
...
function render() {
          ...
          for (let i = 0; i < bundeslaender.length; i++) {
                    ...
                    for (let x = 0; x < land.comments.length; x++) {
                              const element = land.comments[x];

                              document.getElementById(
                                        `comments${i}`
                              ).innerHTML += `${element}<br>`;
                    }
          }
}

function addComment(value) {
          let addComment = document.getElementById(`input${value}`).value;
          bundeslaender[value].comments.push(addComment);
          render();
}
